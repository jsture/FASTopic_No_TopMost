...
import re
import string
import fastopic_no_topmost.topmost.gensim as gensim
from fastopic_no_topmost.topmost.utils import (
    get_stopwords_set,
    Logger,
    read_jsonlist,
    make_dir,
...
class Tokenizer:
    def __init__(
        self, stopwords, keep_num, keep_alphanum, strip_html, no_lower, min_length
    ):
...
def make_word_embeddings(vocab):
    glove_vectors = gensim.load("glove-wiki-gigaword-200")
    word_embeddings = np.zeros((len(vocab), glove_vectors.vectors.shape[1]))  # type: ignore

    num_found = 0

    try:
        key_word_list = glove_vectors.index_to_key  # type: ignore
    except AttributeError:
        key_word_list = glove_vectors.index2word  # type: ignore
...
class Preprocessing:
    def __init__(
        ...
        if tokenizer is not None:
            ...
            self.tokenizer = Tokenizer(
                stopwords, keep_num, keep_alphanum, strip_html, no_lower, min_length
            ).tokenize
    ...
    def parse(self, texts, vocab):
        ...
        bow = bow.toarray()  # type: ignore
        return parsed_texts, bow

    def preprocess_jsonlist(self, dataset_dir, label_name=None):
        train_items = read_jsonlist(os.path.join(dataset_dir, "train.jsonlist"))
        test_items = read_jsonlist(os.path.join(dataset_dir, "test.jsonlist"))

        logger.info(
            f"Found training documents {len(train_items)} testing documents {len(test_items)}"
        )
        ...
        rst = self.preprocess(
            raw_train_texts, train_labels, raw_test_texts, test_labels
        )
    ...
    def preprocess(
        ...
        vocab = [
            ...
            if doc_counts[i] >= self.min_doc_count and doc_freqs[i] <= self.max_doc_freq
        ]
        ...
        train_idx = [
            ...
            if len(text.split()) >= self.min_term
        ]
        ...
        if self.test_sample_size:
            ...
            train_sample_size = int(
                (test_sample_size / self.test_p) * (1 - self.test_p)
            )
            ...
            train_idx = train_idx[
                np.sort(np.random.choice(train_num, train_sample_size, replace=False))
            ]
            test_idx = test_idx[
                np.sort(np.random.choice(test_num, test_sample_size, replace=False))
            ]

            logger.info(f"sampled train size: {len(train_idx)}")
            logger.info(f"sampled train size: {len(test_idx)}")

        train_texts, train_bow = self.parse(
            np.asarray(train_texts)[train_idx].tolist(), vocab
        )
        ...
        logger.info(
            f"Real training size: {len(train_texts)} \t avg length: {rst['train_bow'].sum() / len(train_texts):.3f}"
        )

        if raw_test_texts is not None:
            rst["test_texts"], rst["test_bow"] = self.parse(
                np.asarray(test_texts)[test_idx].tolist(), vocab
            )
            ...
            logger.info(
                f"Real testing size: {len(rst['test_texts'])} \t avg length: {rst['test_bow'].sum() / len(rst['test_texts']):.3f}"
            )
    ...
    def save(
        ...
        save_text(vocab, f"{output_dir}/vocab.txt")
        save_text(train_texts, f"{output_dir}/train_texts.txt")
        scipy.sparse.save_npz(
            f"{output_dir}/train_bow.npz", scipy.sparse.csr_matrix(train_bow)
        )
        ...
        if test_bow is not None:
            scipy.sparse.save_npz(
                f"{output_dir}/test_bow.npz", scipy.sparse.csr_matrix(test_bow)
            )

        if test_texts is not None:
            save_text(test_texts, f"{output_dir}/test_texts.txt")
